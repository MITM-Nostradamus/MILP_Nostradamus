...
def Match_con(model,input_col_white,output_col_white,match_ability):
    ...
    model.addConstr(match_ability == 4 - sum_input_white - sum_output_white)
...
def nostradamus(ROUNDS,initial_round,match_round,TD,quantum):
    ...
    match_state_forward = m.addVars(ROW*COL,vtype=GRB.BINARY, name="match_state_forward")
    match_state_backward = m.addVars(ROW*COL,vtype=GRB.BINARY, name="match_state_backward")
